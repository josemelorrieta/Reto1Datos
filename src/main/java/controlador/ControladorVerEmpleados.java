	...
	private class listenerBotones implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			switch (e.getActionCommand()) {
				case "buscarDNI": vista.pCentral.pVerEmple.txtBusqNombre.setText(""); 
					buscarEmpleado("DNI", vista.pCentral.pVerEmple.txtBusqDNI.getText());
					break;
				case "buscarNombre": vista.pCentral.pVerEmple.txtBusqDNI.setText(""); 
					buscarEmpleado("Nombre", vista.pCentral.pVerEmple.txtBusqNombre.getText());
					break;
				case "ver": int index = vista.pCentral.pVerEmple.listEmpleados.getSelectedIndex();
					if (index != -1) {
					for (Empleado emple: modelo.empleados) {
						if(emple.getDni().equals(vista.pCentral.pVerEmple.listModel.getElementAt(index).substring(0,9))) {
							verDatosEmple(emple);
							break;
						}
					}
	...
	private void buscarEmpleado(String campo, String valor) {
		...
		if (campo.equals("DNI")) {
			...
			if (emples.size() > 0) {
				...
				JOptionPane.showMessageDialog(vista, "No existe ningún empleado con ese DNI", "Información", JOptionPane.INFORMATION_MESSAGE);
			}
