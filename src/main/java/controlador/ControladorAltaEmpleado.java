...
public class ControladorAltaEmpleado {
	...
	private void nuevoEmple() {
		...
		if (validacionDatos(panel.txtCodEmple.getText(), panel.txtNombreEmple.getText(), panel.txtSueldo.getText())) {
			...
			empleados.add(new Empleado(panel.txtCodEmple.getText(), panel.txtNombreEmple.getText(),
				...
				if (validarLetraDNI(dni)) {
					if (sueldo.matches(sueldoRegexp)) {
						...
						JOptionPane.showMessageDialog(this.vista,
								"Debe introducir un sueldo válido (separador decimal . )", "Aviso",
								JOptionPane.WARNING_MESSAGE);
						validacion = false;
					}
				} else {
					JOptionPane.showMessageDialog(this.vista, "La letra del DNI no es correcta", "Aviso",
							JOptionPane.WARNING_MESSAGE);
					validacion = false;
				}
			} else {
				JOptionPane.showMessageDialog(this.vista, "Debe introducir un DNI válido", "Aviso",
						JOptionPane.WARNING_MESSAGE);
				validacion = false;
			}

		} else {
			JOptionPane.showMessageDialog(this.vista, "Debe rellenar todos los campos", "Aviso",
					JOptionPane.WARNING_MESSAGE);
			validacion = false;
		}
	...
	private boolean validarLetraDNI(String dni) {
		...
		if (letras.substring(Integer.parseInt(dni.substring(0, 8)) % 23, Integer.parseInt(dni.substring(0, 8)) % 23 + 1)
				.equals(dni.substring(8, 9)))
			return true;
		else
			return false;
	}
